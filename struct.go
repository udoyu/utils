       ...
       if v.Kind() == reflect.Ptr {
           if v.NumMethod() == 0 {
               if str:=qi.Query(t); len(str)!=0 && v.CanSet() {
                   ...
                   switch v.Type() {
                       case STRING_PTR     : v.Set(reflect.ValueOf(String(str)))
                       case INT32_PTR      : v.Set(reflect.ValueOf(Int32(int32(Atoi(str)))))
                       case INT64_PTR      : v.Set(reflect.ValueOf(Int64(Atol(str))))
                       case UINT32_PTR     : v.Set(reflect.ValueOf(Uint32(uint32(Atoi(str)))))
                       case UINT64_PTR     : v.Set(reflect.ValueOf(Uint64(uint64(Atol(str)))))
                       case BOOL_PTR       : if str == "true" {
